...
The presentation is always native viz cells in an app layout; these differ only in **who
first authors the native SQL cells underneath them** (both end gated the same way):

- **Agent-built SQL (default).** You write the brief; the notebook agent builds the
  **SQL cells, the parameters, the chart cells, and the app layout**. You then run the fidelity gate
...
- **Pre-built SQL.** You build + gate the SQL cells first, then the agent builds the charts
  and layout on top of them. Choose this when the **data population is subtle** (a
  `{% form %}` param that rewrites the `WHERE`, a definition that hides a join, a
  relative-date window) and you want the numbers pinned and gated *before* any chart work,
  or when a tile plots a **ratio of aggregates** and the ratio column has to exist in SQL
  before any chart can bind to it. It costs a
...
**pre-built** for high-stakes, subtle-population, or ratio-bearing reports. Either way,
tell the agent to build **native chart cells that read those SQL dataframes** — and to
consolidate rather than write one pre-aggregated query per chart
...
6. **The charts** — one line per chart: title (exact), **the Hex cell type it becomes**
   (EXPLORE bar/line/area/pie/scatter, `pivot-table`, METRIC, markdown, Python 🐍), x / y +
   aggregation, color/series, sort, number format, and which query/derivation it reads.
   (Big numbers → METRIC fed a **1-row** query.) ⚠️ Mark any tile plotting a **ratio of
   aggregates** — a native chart can't compute it, so the ratio must be a **column
   pre-computed in SQL** at that chart's grain; say so in the brief.
...
8. **Layout** — the row-by-row order the report assembles into (Report Builder rows, or
   the HTML report's structure), **with each tile's width as a share of its row** so the
   agent can map it onto Hex's 0–120 app-layout grid. Say explicitly that the Mode
   source-reference cells and raw SQL cells stay **out** of the app layout.
9. **Styling** — colors per series (hex codes) + number/date formats (high-value
   fidelity); leave cosmetic-only knobs to Hex defaults. Native cells don't take bespoke
   CSS — mirror *structure*, not pixels.
10. **Gaps** — bespoke HTML/CSS report pages, D3/JS embeds, R-only logic, non-warehouse
    sources, maps: name them as known gaps (🐍/⚠️) **with the agreed native substitute**
    (approximating chart, Python cell, markdown, or dropped) so the agent doesn't silently
    invent one. → [`build-classic-app.md`](build-classic-app.md) §Known ceilings.
...
2. **Start the thread with the classic-app prompt.** It must be **prescriptive about cell
   types** — *"Build this as a CLASSIC HEX APP using native notebook cells (SQL, input
   parameters, EXPLORE/METRIC/pivot charts) arranged in the App builder layout. Do NOT
   build a Generative app"* — and tell the agent to build the SQL derivations + params,
   then one native cell per tile per the styling spec, then the `appLayout`. Full prompt
   template → [`build-classic-app.md`](build-classic-app.md). (Pre-built SQL: add *"the SQL
...
4. **Prompt framing decides the output *form* — there's no CLI flag.** Always **verify the
   form** after the build (`hex project export` → `genAppFiles` should be **empty**, native
   `cellType`s present, `appLayout` populated) and re-prompt if it built a generative app.
...
- **Confirm what it built** — `hex project export`: `genAppFiles` **empty**, native
  EXPLORE/METRIC/pivot cells present, `appLayout` populated, SQL cells present +
  (pre-built) unchanged, charts bound to the right dataframes, params wired **upstream**
  of their consumers, and the SQL **consolidated** (not one pre-aggregated query per chart).
- **Run the cell-spec diff** on the viz cells — chart types, encodings, hex colors,
  formats, `seriesId` linkage, layout. Cheap, deterministic, and it catches most
  look-&-feel bugs before you screenshot anything →
  [`build-classic-app.md`](build-classic-app.md).
