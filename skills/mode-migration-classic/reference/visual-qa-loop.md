...
The **render gate for the classic-app build**. It closes the blind spot the rest of the
skill would otherwise punt on ("neither agent can render the app; it's behind login") — it
renders the app headlessly with a **persistent browser profile the customer logs into
once**, diffs it panel-by-panel against the original Mode report, and feeds the notebook
agent surgical fixes until parity.

> **Run the cell-spec diff FIRST.** In a classic app the chart specs are in the exported
> YAML, so wrong chart type, wrong encoding, wrong colors, wrong number format, and a
> broken `seriesId` linkage are all catchable *without rendering anything* →
> [`build-classic-app.md`](build-classic-app.md) §Verify accuracy + fidelity. Come here
> with a clean spec diff, so these rounds are spent on what only a render can reveal:
> **blank or partial series, collapsed chart heights (a fixed `height` on a chart-type
> EXPLORE), clipped or mis-ordered layout rows, overflowing labels, and live filter
> behaviour.** ⚠️ **COMPLETED ≠ renders correctly** — the run oracle passes a broken viz
> spec, which is exactly why this gate isn't optional.
...
4. send ONE coherent fix batch:  hex thread continue <thread_id> "<numbered fix list>"
     (layout-only fixes — row order, column spans, a stray cell in the app — are often
      faster as a direct export → edit `appLayout` → import round-trip; see gotchas.md)
