...
iterate. The deliverable is a **classic app** — native notebook cells in an app layout.
This doc covers the brief and the thread; the prescriptive prompt, the "is it classic?"
verification, the styling spec, and the cell-spec diff live in
[`build-classic-app.md`](build-classic-app.md). Use this doc once you've parsed the
workbook (Phase 1).
...
The presentation is always native viz cells in an app layout; these differ only in
**who first authors the native SQL cells underneath them** (both end gated the same way):

- **Agent-built SQL (default).** You write the brief; the notebook agent builds the
  **SQL cells, the parameters, the chart cells, and the app layout**. You then run the
  fidelity gate **post-hoc** on the SQL it built (read its values, diff against the
  `.twb`). Simplest, and it leans fully on the agent's schema + context sight. Verified
  to reproduce correct, gated SQL from a good brief.
- **Pre-built SQL.** You build + gate the SQL cells first, then the agent builds the
  charts and layout on top of them. Choose this when the **data population is subtle**
  (aggressive shared filters, a fan-out risk, a relative-date window) and you want the
  numbers pinned and gated *before* any chart work — or when a tile plots a **table calc
  or a ratio of aggregates** and that column has to exist in SQL before any chart can
  bind to it. See [`sql-review.md`](sql-review.md). It costs a YAML round-trip (`hex cell
  create` can't mint INPUT or dataframe-SQL companion cells — see
  [`gotchas.md`](gotchas.md)).

When unsure, default to **agent-built** and rely on the post-hoc gate; escalate to
**pre-built** for high-stakes, subtle-population, or table-calc/ratio-bearing workbooks.
Either way, tell the agent to build **native chart cells that read those SQL dataframes**
— and to consolidate rather than write one pre-aggregated query per worksheet
([`build-classic-app.md`](build-classic-app.md)).
...
6. **The charts** — one line per worksheet: title (exact), **the Hex cell type it
   becomes** (EXPLORE bar/line/area/pie/scatter, `pivot-table`, METRIC, markdown,
   Python 🐍), x / y + aggregation, color/series, sort, number format, and which
   derivation it reads. (Big numbers → METRIC fed a **1-row** query.) Resolve caption
   landmines here (name the real field, not the worksheet caption). ⚠️ Mark every tile
   plotting a **table calc, running total, % of total, or ratio of aggregates** — a
   native chart can't compute those, so each must be a **column pre-computed in SQL**
   at that chart's grain; say so in the brief and list it under the derivations (5).
7. **Layout** — the row-by-row order the dashboard should assemble into, **with each
   tile's width as a share of its row** (from the `<zone>` geometry) so the agent can
   map it onto Hex's 0–120 app-layout grid. Say explicitly that the `.twb` source cell
   and the raw SQL cells stay **out** of the app layout.
8. **Styling** — colors per member (hex codes) + number/date formats (high-value
   fidelity); leave cosmetic-only knobs to Hex defaults. Native cells don't take
   bespoke chrome — mirror *structure*, not pixels.
9. **Gaps** — maps, iframe embeds, per-row detail/LOD text tooltips, navigation
   buttons: name them as known gaps (🐍/⚠️) **with the agreed native substitute**
   (approximating chart, Python cell, markdown, or dropped) so the agent doesn't
   silently invent one. → [`build-classic-app.md`](build-classic-app.md) §Known ceilings.
...
2. **Start the thread with the classic-app prompt.** It must be **prescriptive about
   cell types** — *"Build this as a CLASSIC HEX APP using native notebook cells (SQL,
   input parameters, EXPLORE/METRIC/pivot charts) arranged in the App builder layout.
   Do NOT build a Generative app"* — and tell the agent to build the SQL derivations +
   params, then one native cell per tile per the styling spec, then the `appLayout`.
   Full prompt template → [`build-classic-app.md`](build-classic-app.md). (Pre-built
...
4. **Prompt framing decides the output *form* — there's no CLI flag.** Opening with
   "Build this as a GENERATIVE APP…" yields a custom code app (`genAppFiles`); a
   **chart-type-prescriptive** prompt yields native cells. Always **verify the form**
   after the build (`hex project export` → `genAppFiles` should be **empty**, native
   `cellType`s present, `appLayout` populated) and re-prompt if it built a generative
   app. Full detail → [`build-classic-app.md`](build-classic-app.md).
...
- **Confirm what it built** — `hex project export`: `genAppFiles` **empty** (it's
  classic, not generative), native EXPLORE/METRIC/pivot cells present, `appLayout`
  populated, SQL cells present + (pre-built) unchanged, charts bound to the right
  dataframes, params wired **upstream** of their consumers, and the SQL
  **consolidated** (not one pre-aggregated query per worksheet).
...
- **Run the cell-spec diff** on the viz cells — chart types, encodings, hex colors,
  formats, date-axis `dataType`, `seriesId` linkage, layout. Cheap, deterministic, and
  it catches most look-&-feel bugs before you screenshot anything →
  [`build-classic-app.md`](build-classic-app.md).
