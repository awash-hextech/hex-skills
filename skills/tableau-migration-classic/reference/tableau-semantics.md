...
(that plan → a classic app of native Hex cells) lives in `build-classic-app.md`, with
the native-cell capability map in `building-cells.md`.

⚠️ **In a classic build, Phase 1 carries more weight than it does in the generative
one.** A native chart cell has no window functions and no calculated measures, so every
**table calc (§4), LOD ratio (§3), percent-of-total and ratio-of-aggregates (§9)** must
leave Phase 1 as a **column pre-computed in SQL** at the consuming chart's grain. If it
isn't in the SQL, no chart can plot it.
