...
into an intent-level plan for the brief. Phase 2 (that plan → native Hex cells) lives
in `building-cells.md`.
...
| `mode.export_html(...)` / HTML output embedded in the report | render in a Python cell, or fold into the generative app |
...
## 5. The presentation layer → the generative app

Mode reports present in one of two ways; both target the **generative app**:

- **Report Builder** (drag-and-drop layout of charts, big numbers, text, filters) →
  describe the row-by-row layout + filter wiring in the brief; the generative app
  reproduces it.
- **Bespoke HTML / Liquid report page** (custom HTML/CSS, `{% for row in query %}` loops,
  Chart.js/D3 embeds) → this is exactly what the generative app is for. Extract the layout,
  the CSS colors (hex codes), and the data bindings; describe them in the styling spec.
  Arbitrary hand-written D3/JS is **best-effort** — flag anything the generative app can't
  faithfully reproduce (⚠️).

Report-level **filters** (Mode's filter bar) → Hex input parameters + cross-filter,
wired per the styling spec.
