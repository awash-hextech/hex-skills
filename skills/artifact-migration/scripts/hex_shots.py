#!/usr/bin/env python3
"""Headless screenshot of a Hex app for the visual-QA loop.
...
Match the source capture's theme/viewport when the artifact shipped dark mode or
was responsive, so the diff compares like with like:
    python scripts/hex_shots.py "<url>" --theme dark       -o migrated-dark.png
    python scripts/hex_shots.py "<url>" --viewport 390x844 -o migrated-mobile.png
...
def capture(url, out, viewport, theme):
    ...
    with sync_playwright() as p:
        ctx = p.chromium.launch_persistent_context(
            str(PROFILE), headless=True, viewport=viewport, color_scheme=theme
        )
        page = ctx.pages[0] if ctx.pages else ctx.new_page()
